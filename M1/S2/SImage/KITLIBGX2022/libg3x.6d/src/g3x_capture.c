...
extern bool g3x_PlugCapture(const char *basename, int dlx, int dly, int urx, int ury)
{
  ...
  { fprintf(stderr,"\e[1;31m<g3x_PlugCapture> : Erreur Creation fichier temporaire <%s>\e[0m\n",tempsnapname); return false;}
  return true;
}
...
extern bool g3x_Snapshot(const char *format, const char *basename, int w, int h)
{
  ...
  if (match_string(format,pnm_name))
    ...
    snprintf(command,1023,"cat %s | pnmflip -tb | pnmtops -noturn -rle -dpi 1200 > %s",tempsnapname,snapname);
  else
  {
...
extern bool g3x_FilmFrame(void)
{
  ...
  {
    if (-1==mkdir(tempdirname,S_IRWXU)) { perror("mkdir"); return false; }
