...
static QuadNode QT;
static Obj      PTL[N];
static G2Xcolor PCOL[N];
static G2Xcolor QCOL[N];
static G2Xpnm*  pnm=NULL;

static void change_node_list(Obj *p, QuadNode *dad, QuadNode *son)
...
static bool point_in_4node(QuadNode *node, Obj *p)
{ /* renvoie True si le point 'p->pos' est dans le noeud 'node' */
  if ( (fabs(node->C.x-p->pos.x) > node->dim.x) ||
       (fabs(node->C.y-p->pos.y) > node->dim.y) ) return false;
  return true;
}

static void set_sons_pt(QuadNode *dad, QuadNode *son)
{ /* crée la liste des points dans <node> à partir de */
  Obj *p=dad->ptl,*next=NULL;
...
static void Init(void)
{
  ...
  if (!g2x_PnmRead(&pnm,"Monarch.512.ppm",false)) g2x_Quit();
  ...
  g2x_ColorMapHSV360(PCOL,N);
  g2x_ColorMapHSV360_va(QCOL,N,1.,0.9);
}
...
static void Show_Quadtree(QuadNode *node)
{
  G2Xcolor col;
  if (!node) return;
  if (node->nbp==1)
  {
    ...
    g2x_TexRectangle(node->DL.x,node->DL.y,node->UR.x,node->UR.y, pnm->map,pnm->height,pnm->width);
    g2x_Rectangle(node->DL.x,node->DL.y,node->UR.x,node->UR.y,G2Xwa,1);
  }
  int i;
  for (i=0;i<DIM;i++)
    Show_Quadtree(node->son[i]);
}
