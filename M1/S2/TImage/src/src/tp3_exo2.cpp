...
cv::Mat computeCumulatedHistgramGS(const cv::Mat& imageGS) {
  cv::Mat myHistogram(256, 1, CV_32S, cv::Scalar(0));
  cv::Mat histogram = computeHistogramGS(imageGS);

  int somme = 0;
  for (int i = 0; i < 256; i++) {
    somme += histogram.at<int>(i);
    myHistogram.at<int>(i) = somme;

  }
  return myHistogram;
}

cv::Mat generateLUTHistogramGS(const cv::Mat& imageGS) {
  cv::Mat myLUT(256, 1, CV_8U);
  cv::Mat histogram = computeCumulatedHistgramGS(imageGS);

  for(int i = 0; i < 256; i++) {
    myLUT.at<unsigned char>(i) = ((255 * histogram.at<int>(i)) / (imageGS.rows * imageGS.cols)); 
  }
  return myLUT;
}
...
int main(int argc, char ** argv)
{
  ...
  // normalize the image
  cv::Mat normalizeImage = apply_LUT(image, generateLUTHistogramGS(image));

  // display an image
  std::cout << "appuyer sur une touche ..." << std::endl;
  cv::imshow("image", normalizeImage);
  cv::imshow("histogramme", histogramToImageGS(computeCumulatedHistgramGS(normalizeImage)));
  cv::waitKey();
